...
ui <- dashboardPage(
  ...
  dashboardBody(tabItems(
    ...
    tabItem(
      ...
      fluidRow(box(
        column(2, selectInput("major", "대분류:", c("All"))), 
        column(2, selectInput("name", "품명:", c("All"))), 
        column(2, selectInput("std", "규격:", c("All"))), 
        column(2, selectInput("year", " 연도:", c("All"))), 
        column(2, selectInput("site", "현장:", c("All"))), 
        column(2, selectInput("coop", "협력사:", c("All"))), 
        width = NULL
      )),
    ...
    tabItem(
      ...
      fluidRow( 
        # Input: Select a file
        box(fileInput("sheet", "Choose CSV File", multiple = FALSE,
...
server <- function(input, output, session) {
  observe({
    ...
    output$table <- DT::renderDataTable(datatable(table, extensions = "FixedHeader", 
                                                  options = list(fixedHeader = T, lengthMenu = list(c(10, 25, 50, 100, -1), c("10", "25", "50", "100", "All"))))
    ...
    updateSelectInput(session, inputId = "major", choices = c("All", sort(unique(as.character(list.select[[1]]$대분류)))), selected = input$major)
    updateSelectInput(session, inputId = "name", choices = c("All", sort(unique(as.character(list.select[[2]]$품명)))), selected = input$name)
    updateSelectInput(session, inputId = "std", choices = c("All", sort(unique(as.character(list.select[[3]]$규격)))), selected = input$std)
    updateSelectInput(session, inputId = "year", choices = c("All", sort(unique(as.character(list.select[[4]]$연도)))), selected = input$year)
    updateSelectInput(session, inputId = "site", choices = c("All", sort(unique(as.character(list.select[[5]]$현장)))), selected = input$site)
    updateSelectInput(session, inputId = "coop", choices = c("All", sort(unique(as.character(list.select[[6]]$협력사)))), selected = input$coop)
    
    # Read uploaded sheet
    req(input$sheet)
    tryCatch(
      {
        sheet <- read.csv(input$sheet$datapath, stringsAsFactors = F, fileEncoding = "euc-kr")
      },
      error = function(e) {
    ...
    output$stat <- DT::renderDataTable(datatable(stat, extensions = "FixedHeader", 
                                                 options = list(fixedHeader = T, lengthMenu = list(c(10, 25, 50, 100, -1), c("10", "25", "50", "100", "All")), pageLength = 25))
    ...
    output$sheet <- DT::renderDataTable(datatable(sheet, extensions = "FixedHeader", 
                                                  options = list(fixedHeader = T, lengthMenu = list(c(10, 25, 50, 100, -1), c("10", "25", "50", "100", "All")), pageLength = 25, searching = FALSE))
